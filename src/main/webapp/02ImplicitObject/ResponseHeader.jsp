...
<%
	...
	// 문자열 -> 숫자 변경
	response.addIntHeader("resp_int", add_int); // 응답헤더에 resp_int 로 정수 추가
	response.addHeader("resp_str", request.getParameter("add_str")); // 추가
	response.setHeader("resp_str", "아무개"); // 수정
%>
<!DOCTYPE html><html><head><meta charset="UTF-8">
<title>ResponseHeader.jsp : get 으로 받은 날짜, 숫자, 문자 처리</title>
</head>
<body>
	<h2> ResponseMain.jsp 에서 넘어온 응답헤더 출력</h2>
	<%
	...
	%>
			<li> <%= hName %> : <%= hValue %> </li> <!-- for 중간에 태그 사용 -->
	<%	
		} // for 종료 
	%>
